...
|----|---------------------------------------------------------------------------|------------------|-----------------------------------------------------|
| 1  | Сервис запускается по инструкциям из `project/README.md` и работает      | ✅               | `README.md`, раздел «Как запустить»; проверено локально через `uvicorn` |
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        | ✅               | `src/service/main.py` (загружает `artifacts/model.joblib`), `src/models/train.py` |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          | ✅               | `notebooks/01_eda.ipynb`, `report.md` (раздел 3)     |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        | ✅               | `notebooks/02_baselines.ipynb`, `report.md` (Таблица 2.1) |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            | ✅               | `src/data/`, `src/models/`, `src/service/` |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       | ✅               | `Dockerfile`, `.dockerignore`, а также шаги без Docker в `README.md` |
| 7  | Есть `.env.example` и **нет** в репозитории реальных секретов/паролей    | ✅               | `.env.example` (в `project/`), `SECURITY.md` в корне репозитория, отсутствие `.env` |
| 8  | Реализованы логи/наблюдаемость (хотя бы консольные логи + `/health`)     | ✅               | `src/service/main.py` (middleware логирования запросов), endpoint `/health` |
