...
# ---------- /quality-flags-from-csv: возврат всех флагов качества ----------
...
class QualityFlagsResponse(BaseModel):
    ...
    flags: dict[str, bool] = Field(
        ...,
        description="Полный набор булевых флагов качества из HW03",
    )
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт для получения всех флагов качества датасета из CSV.
    Включает флаги из HW03: константные колонки, высокая кардинальность,
    большое количество нулей и другие эвристики.
    """
