...
def report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть колонки с большой долей нулей: **{quality_flags['has_many_zero_values']}**\n")
        ...
        if missing_df.empty:
            ...
            if not problem_cols.empty:
                ...
                for col_name in problem_cols.index:
                    share = problem_cols.loc[col_name, 'missing_share']
        ...
        if not top_cats:
            ...
            f.write(f"Показано top-{top_k_categories} значений для категориальных признаков.\n\n")
            f.write("См. файлы в папке `top_categories/`.\n\n")

        f.write("## Гистограммы числовых колонок\n\n")
        f.write(f"Максимум {max_hist_columns} колонок с гистограммами.\n\n")
    ...
    if json_summary:
        problem_cols = []
        if not missing_df.empty:
            problem_missing = missing_df[missing_df['missing_share'] > min_missing_share]
            for col_name in problem_missing.index:
                problem_cols.append({
                    "column": col_name,
                    "issue": "high_missing",
                    "missing_share": float(problem_missing.loc[col_name, 'missing_share'])
                })

        if quality_flags.get('constant_columns'):
            for col in quality_flags['constant_columns']:
                problem_cols.append({"column": col, "issue": "constant"})

        if quality_flags.get('high_cardinality_columns'):
            for col in quality_flags['high_cardinality_columns']:
                problem_cols.append({"column": col, "issue": "high_cardinality"})

        if quality_flags.get('zero_heavy_columns'):
            for col in quality_flags['zero_heavy_columns']:
                problem_cols.append({"column": col, "issue": "many_zeros"})
